...
s <- summary(nw~sum+test.abs.sum.minus.5+test.abs.sum.minus.5(FALSE), response="v")
stopifnot(all(abs(s[1]-5)==s[2]) && all(abs(s[1]-5)==s[3]))
s <- simulate(nw~sum,monitor=~test.abs.sum.minus.5+test.abs.sum.minus.5(FALSE), response="v", reference=~Poisson, statsonly=TRUE,coef=0, nsim=1000, control=control.simulate.formula(MCMC.burnin=0,MCMC.interval=1))
stopifnot(all(abs(s[,1]-5)==s[,2]) && all(abs(s[,1]-5)==s[,3]))
