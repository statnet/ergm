#  File tests/testthat/test-bd.R in package ergm, part of the Statnet suite of
...
test_that("Bounded degree (bd()) maximum constraint for undirected networks", {
  ...
  expect_message(expect_gt(approx.hotelling.diff.test(constr,degr[,-2])$p.value, 0.01),
                 "^Data are not full-rank.*")
...
test_that("Bounded degree (bd()) constraints for directed networks", {
  ...
  expect_message(expect_gt(approx.hotelling.diff.test(constr,degr[,-(2:4)])$p.value, 0.01),
                 "^Data are not full-rank.*")
