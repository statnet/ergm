...
test_that("Bounded degree (bd()) maximum constraint for undirected networks", {
  ...
  expect_warning(expect_true(approx.hotelling.diff.test(constr,degr[,-2])$p.value > 0.01), ".*do not vary but equal mu0.*")
...
test_that("Bounded degree (bd()) constraints for directed networks", {
  ...
  expect_warning(expect_true(approx.hotelling.diff.test(constr,degr[,-(2:4)])$p.value > 0.01), ".*do not vary but equal mu0.*")
