...
test_that("Nonidentifiable model produces a warning.", {
  ...
  expect_warning(ergm(flomarriage~edges+nodecov(~wealth)+nodecov(~-wealth/2+1)+gwesp(fixed=FALSE), control=control.ergm(MCMLE.maxit=1)),".*\\bsingular\\b.*\\bnonidentifiable\\b.*")
})
