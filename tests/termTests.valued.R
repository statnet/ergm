...
# nodematch
tst(sum(abs(outer(f,f,"=="))*dirm,na.rm=TRUE), summary(dirnw ~ nodematch("q"), response="w"))
tst(sum(abs(outer(f,f,"=="))*(dirm!=0),na.rm=TRUE), summary(dirnw ~ nodematch("q", form="nonzero"), response="w"))
# TODO: nodematch with diff=TRUE
