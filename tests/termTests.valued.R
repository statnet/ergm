...
for(base in list(0, 1, 2, 1:2, 3)){
  ...
  tst(sapply(sort(unique(f1))[keep], function(x) sum((f1==x)*bipm,na.rm=TRUE)), summary(bipnw ~ b1factor("f", base=NULL, levels=keep), response="w"))
...
for(base in list(0, 1, 2, 1:2, 3)){
  keep <- if(all(base==0)) 1:3 else (1:3)[-base]
  tst(sapply(sort(unique(f))[keep], function(x) sum((f==x)*(dirm+t(dirm)),na.rm=TRUE)), summary(dirnw ~ nodefactor("f", base=NULL, levels=keep), response="w"))
  ...
  tst(sapply(sort(unique(f))[keep], function(x) sum((f==x)*undm,na.rm=TRUE)), summary(undnw ~ nodefactor("f", base=NULL, levels=keep), response="w"))
  ...
  tst(sapply(sort(unique(f))[keep], function(x) sum((f1==x)*bipm+t((f2==x)*t(bipm)),na.rm=TRUE)), summary(bipnw ~ nodefactor("f", base=NULL, levels=keep), response="w"))
...
for(base in list(0, 1, 2, 1:2, 3)){
  keep <- if(all(base==0)) 1:3 else (1:3)[-base]
  tst(sapply(sort(unique(f))[keep], function(x) sum((f==x)*t(dirm),na.rm=TRUE)), summary(dirnw ~ nodeifactor("f", base=NULL, levels=keep), response="w"))
...
for(base in list(0, 1, 2, 1:2, 3)){
  keep <- if(all(base==0)) 1:3 else (1:3)[-base]
  tst(sapply(sort(unique(f))[keep], function(x) sum((f==x)*dirm,na.rm=TRUE)), summary(dirnw ~ nodeofactor(~f, base=NULL, levels=keep), response="w"))
