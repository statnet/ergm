...
for(coef in coefs){
  ...
  if(test$p.value<.001) {print(test); stop("Simulation test failed (mean).")}
  test <- approx.hotelling.diff.test((s-E(coef,a,b)*6)^2/6,mu0=V(coef,a,b))
  if(test$p.value<.001) {print(test); stop("Simulation test failed (variance).")}
