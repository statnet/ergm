/*  File src/MCMC.c.template.do_not_include_directly.h in package ergm, part of
...
/*****************
 void ETYPE(MCMC_wrapper)

 Wrapper for a call from R.

 and don't forget that tail -> head
*****************/
SEXP ETYPE(MCMC_wrapper)(SEXP stateR,
                    // MCMC settings
  ...
  SEXP status;
  if(MHp) status = PROTECT(ScalarInteger(ETYPE(MCMCSample)(s,
...
/*  Catch more edges than we can return */
  ...
  if(s->save){
    ...
    SET_VECTOR_ELT(s->save, 0, ETYPE(ErgmStateRSave)(s));
  }
...
/*   if (verbose){
  ...
  if (samplesize>1){
	...
	return MCMC_TOO_MANY_EDGES;
      ...
      if(s->save){
        s->stats = networkstatistics;
        SET_VECTOR_ELT(s->save, i, ETYPE(ErgmStateRSave)(s));
      }
...
/*********************
 ...
 In this function, eta is a m->n_stats-vector just as in ETYPE(MCMCSample),
...
SEXP ETYPE(MCMCPhase12) (SEXP stateR,
                           // Phase12 settings
                           SEXP theta0,
  ...
  SEXP status;
  if(MHp) status = PROTECT(ScalarInteger(ETYPE(MCMCSamplePhase12)(s,
...
MCMCStatus ETYPE(MCMCSamplePhase12)(ETYPE(ErgmState) *s,
  ...
  MCMCStatus status = ETYPE(MetropolisHastings)(s, eta,
  ...
  for(unsigned int subphase = 1; subphase <= nsubphases; subphase++){
    ...
    for(unsigned int i=1; ; i++){
      MCMCStatus status = ETYPE(MetropolisHastings)(s, eta, s->stats, interval, &staken,verbose); /*Take a sample network*/
      if(status!=MCMC_OK) return status;
