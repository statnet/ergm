...
/*****************
 void EDGETYPE(MCMC_wrapper)

 Wrapper for a call from R.

 and don't forget that tail -> head
*****************/
SEXP EDGETYPE(MCMC_wrapper)(SEXP stateR,
                    // MCMC settings
  ...
  SEXP status;
  if(MHp) status = PROTECT(ScalarInteger(EDGETYPE(MCMCSample)(s,
...
/*  Catch more edges than we can return */
  if(EDGETYPE(MetropolisHastings)(s, eta, networkstatistics, burnin, &staken,verbose)!=MCMC_OK)
  ...
  if(s->save){
    ...
    SET_VECTOR_ELT(s->save, 0, EDGETYPE(ErgmStateRSave)(s));
  }
...
/*   if (verbose){
  ...
  if (samplesize>1){
    ...
    for (unsigned int i=1; i < samplesize; i++){
      ...
      if(EDGETYPE(MetropolisHastings)(s, eta, networkstatistics, interval, &staken,
			    verbose)!=MCMC_OK)
	return MCMC_MH_FAILED;
      if(nmax!=0 && EDGECOUNT(nwp) >= nmax-1){
	return MCMC_TOO_MANY_EDGES;
      ...
      if(s->save){
        s->stats = networkstatistics;
        SET_VECTOR_ELT(s->save, i, EDGETYPE(ErgmStateRSave)(s));
      }
...
/*********************
 ...
 In this function, eta is a m->n_stats-vector just as in EDGETYPE(MCMCSample),
...
SEXP EDGETYPE(MCMCPhase12) (SEXP stateR,
                           // Phase12 settings
                           SEXP theta0,
  ...
  SEXP status;
  if(MHp) status = PROTECT(ScalarInteger(EDGETYPE(MCMCSamplePhase12)(s,
...
MCMCStatus EDGETYPE(MCMCSamplePhase12)(EDGETYPE(ErgmState) *s,
  ...
  MCMCStatus status = EDGETYPE(MetropolisHastings)(s, eta,
  ...
  for(unsigned int subphase = 1; subphase <= nsubphases; subphase++){
    ...
    for(unsigned int i=1; ; i++){
      MCMCStatus status = EDGETYPE(MetropolisHastings)(s, eta, s->stats, interval, &staken,verbose); /*Take a sample network*/
      if(status!=MCMC_OK) return status;
