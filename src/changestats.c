...
D_CHANGESTAT_FN(d_absdiffcat) { 
  ...
  /* *** don't forget tail -> head */
  FOR_EACH_TOGGLE(i) {
    change = IS_OUTEDGE(tail=tails[i], head=heads[i]) ? -1.0 : 1.0;
...
D_CHANGESTAT_FN(d_altkstar) { 
  ...
  /* *** don't forget tail -> head */
  FOR_EACH_TOGGLE(i) {
    isedge = IS_OUTEDGE(tail=tails[i], head=heads[i]);
...
D_CHANGESTAT_FN(d_b1concurrent) { 
  ...
  FOR_EACH_TOGGLE(i) {
    b1 = tails[i];
    echange = IS_OUTEDGE(b1, heads[i]) ? -1 : 1;
    b1deg = OUT_DEG[b1];
...
D_CHANGESTAT_FN(d_b1factor) { 
  ...
  FOR_EACH_TOGGLE(i) {
    b1 = tails[i];
    s = IS_OUTEDGE(b1, heads[i]) ? -1.0 : 1.0;
    for (j=0; j<(N_CHANGE_STATS); j++) {
...
D_CHANGESTAT_FN(d_b1starmix) { 
  ...
  FOR_EACH_TOGGLE(i) {
    ...
    edgeflag = IS_OUTEDGE(tail = tails[i], head = heads[i]);
...
D_CHANGESTAT_FN(d_b2concurrent) { 
  ...
  FOR_EACH_TOGGLE(i) {
    b2 = heads[i];
    echange = IS_OUTEDGE(tails[i], b2) ? -1 : 1;
    b2deg = IN_DEG[b2];
...
D_CHANGESTAT_FN(d_b2factor) { 
  ...
  FOR_EACH_TOGGLE(i) {
    b2 = heads[i];
    s = IS_OUTEDGE(tails[i], b2) ? -1.0 : 1.0;
    for (j=0; j<(N_CHANGE_STATS); j++) {
      factorval = (INPUT_PARAM[j]);
...
D_CHANGESTAT_FN(d_boundedidegree) { 
  ...
  FOR_EACH_TOGGLE(i) {
    tail = tails[i];
    echange = IS_OUTEDGE(tail, heads[i]) ? -1 : 1;
    taild = IN_DEG[tail];
    for(j = 0; j < N_CHANGE_STATS; j++) {
      deg = (Vertex)INPUT_PARAM[j];
      CHANGE_STAT[j] += (taild + echange == deg) - (taild == deg);
...
D_CHANGESTAT_FN(d_boundedistar) { 
  ...
  FOR_EACH_TOGGLE(i) {
    ...
    head = heads[i];
    edgeflag = IS_OUTEDGE(tails[i], head);
    headod = IN_DEG[head];
...
D_CHANGESTAT_FN(d_boundedostar) { 
  ...
  FOR_EACH_TOGGLE(i) {
    /* is there an edge for this toggle */
    tail = tails[i];
    edgeflag = IS_OUTEDGE(tail, heads[i]);
    tailod = OUT_DEG[tail];
    newtailod = tailod + (edgeflag ? -1 : 1);
      for(j=0; j < p; j++) {
        k =  ((int)INPUT_PARAM[j]);
...
D_CHANGESTAT_FN(d_cycle) { 
  ...
  FOR_EACH_TOGGLE(i) {
    ...
    /*edgeflag = (EdgetreeSearch(tail=tails[i], head=heads[i], g.outedges) != 0);*/
...
D_CHANGESTAT_FN(d_degree) { 
  ...
  ZERO_ALL_CHANGESTATS(i);
  FOR_EACH_TOGGLE(i) {
    echange=(EdgetreeSearch(tail=tails[i], head=heads[i], nwp->outedges)==0)? 1:-1;
...
D_CHANGESTAT_FN(d_degree_by_attr) { 
  ...
  */
  ...
  ZERO_ALL_CHANGESTATS(i);
  FOR_EACH_TOGGLE(i) {
    echange = IS_OUTEDGE(tail=tails[i], head=heads[i]) ? -1:1;
    taildeg = od[tail] + id[tail];
    headdeg = od[head] + id[head];
...
D_CHANGESTAT_FN(d_density) {
  ...
  CHANGE_STAT[0] = 0.0;
  FOR_EACH_TOGGLE(i) {
    CHANGE_STAT[0] += IS_OUTEDGE(tails[i], heads[i]) ? - 1 : 1;
    TOGGLE_IF_MORE_TO_COME(i);
  }
...
D_CHANGESTAT_FN(d_gwb1degree) { 
  ...
  */
  ...
  CHANGE_STAT[0] = 0.0;
  FOR_EACH_TOGGLE(i) {
    echange=(EdgetreeSearch(b1=tails[i], heads[i], nwp->outedges)==0) ? 1 : -1;
...
D_CHANGESTAT_FN(d_gwdsp) { 
  ...
  FOR_EACH_TOGGLE(i){      
    cumchange=0.0;
    ochange = (EdgetreeSearch(tail=tails[i], head=heads[i], nwp->outedges) == 0) ? 0 : -1;
    echange = 2*ochange + 1;
    /* step through outedges of head */
    for(e = EdgetreeMinimum(nwp->outedges, head);
    (u = nwp->outedges[e].value) != 0;
...
D_CHANGESTAT_FN(d_gwb2degree) { 
  ...
  */
  ...
  CHANGE_STAT[0] = 0.0;
  FOR_EACH_TOGGLE(i) {      
    echange=(EdgetreeSearch(tails[i], b2=heads[i], nwp->outedges)==0) ? 1 : -1;
...
D_CHANGESTAT_FN(d_gwidegree) { 
  ...
  FOR_EACH_TOGGLE(i) {
    head=heads[i];
    edgeflag = IS_OUTEDGE(tails[i], head); /* either 0 or 1 */
...
D_CHANGESTAT_FN(d_gwidegree_by_attr) { 
  ...
  */
  ...
  FOR_EACH_TOGGLE(i) {
    head = heads[i];
    echange = IS_OUTEDGE(tails[i], head) ? -1 : 1;
...
D_CHANGESTAT_FN(d_gwodegree) { 
  ...
  FOR_EACH_TOGGLE(i) {
    tail=tails[i];
    edgeflag = IS_OUTEDGE(tail, heads[i]); /* either 0 or 1 */
...
D_CHANGESTAT_FN(d_gwtdsp) {
  ...
  FOR_EACH_TOGGLE(i){
    ...
    for(e = MIN_OUTEDGE(head); (u=OUTVAL(e))!=0; e=NEXT_OUTEDGE(e)) { 
      if (u != tail){
        L2hu=ochange; /* L2hu will be # shrd prtnrs of (tail,u) not incl. head */
...
D_CHANGESTAT_FN(d_gwtesp) { 
  ...
  FOR_EACH_TOGGLE(i){      
	...
	for(f = EdgetreeMinimum(nwp->inedges, u); 
	    (v = nwp->inedges[f].value) != 0;
	    f = EdgetreeSuccessor(nwp->inedges, f)){
	  if(EdgetreeSearch(tail,v,nwp->outedges)!= 0) L2hu++;
	}
	cumchange += pow(oneexpa,(double)L2hu);
      }
    }
	...
	for(f = EdgetreeMinimum(nwp->outedges, u);
	    (v = nwp->outedges[f].value) != 0;
	    f = EdgetreeSuccessor(nwp->outedges, f)){
	  if(EdgetreeSearch(v,head,nwp->outedges)!= 0) L2ut++;
	}
	cumchange += pow(oneexpa,(double)L2ut) ;
      }
    }
...
D_CHANGESTAT_FN(d_hamhamming) { 
  ...
  /* *** don't forget tail -> head */    
  FOR_EACH_TOGGLE(i) {
    discord=IS_OUTEDGE(tail = tails[i], head = heads[i]);
...
/*  edgeflag =(EdgetreeSearch(tail=tails[i], head=heads[i], nwp[0].outedges) != 0); */
    discord=(EdgetreeSearch(tail=tails[i], head=heads[i], nwp[1].outedges) != 0);
...
/*    CHANGE_STAT[0] += ((edgeflag0!=edgeflag) ? -1.0 : 1.0); */
    ...
    if (i+1 < ntoggles){
      ToggleEdge(tails[i], heads[i], &nwp[0]);  /* Toggle this edge if more to come */
      ToggleEdge(tails[i], heads[i], &nwp[1]);  /* Toggle the discord for this edge */
    }
  }
  i--;
  while (--i>=0){  /*  Undo all previous toggles. */
    ToggleEdge(tails[i], heads[i], &nwp[0]);
    ToggleEdge(tails[i], heads[i], &nwp[1]);  /* Toggle the discord for this edge */
  }
}
...
/*  Rprintf("nhedge %d\n", nhedge); */
  if(ntoggles==2){
   matchvaltail = INPUT_PARAM[tails[0]+2*nhedge];
   matchvalhead = INPUT_PARAM[heads[0]+2*nhedge];
   if(matchvaltail != INPUT_PARAM[tails[1]+2*nhedge] ||
      matchvalhead != INPUT_PARAM[heads[1]+2*nhedge]){
...
D_CHANGESTAT_FN(d_idegree_by_attr) { 
  ...
  */
  ...
  ZERO_ALL_CHANGESTATS(i);
  FOR_EACH_TOGGLE(i) {
    echange=(EdgetreeSearch(tails[i], head=heads[i], nwp->outedges)==0)? 1:-1;
    headdeg = id[head];
    headattr = INPUT_PARAM[2*N_CHANGE_STATS + head - 1]; 
    for(j = 0; j < N_CHANGE_STATS; j++) {
      d = (Vertex)INPUT_PARAM[2*j];
  ...
  FOR_EACH_TOGGLE(i) 
    {
      ...
      if(INPUT_PARAM[1+(heads[i]-1)+(tails[i]-1)*nmat] != 0){
	for(e = EdgetreeMinimum(nwp->outedges, head);
	    (node3 = nwp->outedges[e].value) != 0;
	    e = EdgetreeSuccessor(nwp->outedges, e)) /* step through outedges of head */
	  {
	    if(INPUT_PARAM[1+(node3-1)+(tails[i]-1)*nmat] != 0 && 
	       INPUT_PARAM[1+(node3-1)+(heads[i]-1)*nmat] != 0 ){
	      if (DIRECTED){
...
D_CHANGESTAT_FN(d_m2star) {
  ...
  for (i=0; i < ntoggles; i++)
    {
      /*  edgeflag is 1 if the edge from tails[i] to heads[i]  */
...
D_CHANGESTAT_FN(d_meandeg) {
  ...
  CHANGE_STAT[0] = 0.0;
  FOR_EACH_TOGGLE(i) {
    CHANGE_STAT[0] += (IS_OUTEDGE(tails[i], heads[i]) ? -2.0 : 2.0);
    TOGGLE_IF_MORE_TO_COME(i);
  }
  ...
  FOR_EACH_TOGGLE(i) {
    ...
    if (IS_OUTEDGE(head,tail)) { /* otherwise, no change occurs */
      ...
      if (noattr) { /* "plain vanilla" mutual, without node attributes */
        ...
        matchval = INPUT_PARAM[tail+ninputs-1];
        if (matchval == INPUT_PARAM[tail+ninputs-1]) { /* We have a match! */
          if (ninputs==0) {/* diff=F in network statistic specification */
            CHANGE_STAT[0] += change;
          } else { /* diff=T */
            for (j=0; j<ninputs; j++) {
...
D_CHANGESTAT_FN(d_nodeifactor) { 
  ...
  FOR_EACH_TOGGLE(i) {
    head = heads[i];
    s = IS_OUTEDGE(tails[i], head) ? -1.0 : 1.0;
    headattr = INPUT_ATTRIB[head-1];
    for (j=0; j < N_CHANGE_STATS; j++) {
...
D_CHANGESTAT_FN(d_nodeofactor) { 
  ...
  FOR_EACH_TOGGLE(i) {
    tail = tails[i];
    s = IS_OUTEDGE(tail, heads[i]) ? -1.0 : 1.0;
    tailattr = INPUT_ATTRIB[tail-1];
    for (j=0; j < N_CHANGE_STATS; j++) {
...
D_CHANGESTAT_FN(d_odegree_by_attr) { 
  ...
  */
  ...
  ZERO_ALL_CHANGESTATS(i);
  FOR_EACH_TOGGLE(i) {
    echange=(EdgetreeSearch(tail=tails[i], heads[i], nwp->outedges)==0)? 1:-1;
    taildeg = od[tail];
    tailattr = INPUT_PARAM[2*N_CHANGE_STATS + tail - 1]; 
    for(j = 0; j < N_CHANGE_STATS; j++) {
      d = (Vertex)INPUT_PARAM[2*j];
...
D_CHANGESTAT_FN(d_tdsp) {
  ...
  FOR_EACH_TOGGLE(i){
    ...
    for(e = MIN_OUTEDGE(head); (u=OUTVAL(e))!=0; e=NEXT_OUTEDGE(e)) { 
      if (u != tail){
        L2hu=0; /* This will be # of shared partners of (tail,u) */
        /* step through inedges of u, incl. (head,u) itself */
        for(f = MIN_INEDGE(u); (v=INVAL(f))!=0; f=NEXT_INEDGE(f)) {
          if(IS_OUTEDGE(tail,v)) L2hu++;
        }
    ...
    for(e = MIN_INEDGE(tail); (u=INVAL(e))!=0; e=NEXT_INEDGE(e)) {
      if (u != head){
        L2ut=0; /* This will be # of shared partners of (u,head) */
        /* step through outedges of u , incl. (u,tail) itself */
        for(f = MIN_OUTEDGE(u);(v=OUTVAL(f))!=0; f=NEXT_OUTEDGE(f)){
          if(IS_OUTEDGE(v,head)) L2ut++;
        }
