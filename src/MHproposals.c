...
/********************
   void MH_TriNT2
   Tie/no tie with triad: Gives at least 50% chance of
   proposing a toggle of an existing edge, as opposed to simple random
   toggles that rarely do so in sparse networks; then 1/4 of the time
   also proposes a toggle on an edge incident on the tail, 1/4 on the
   head, and 1/4 on both (a triad).

   Then, proposes toggles to "balance" those ties as needed.
***********************/
void MH_TriNT2 (MHproposal *MHp, Network *nwp)
{
  /* *** don't forget tail-> head now */
  ...
  BD_LOOP({
	...
	unsigned int nrebal = MIN(abs(bal), bal>0 ? nedges : ndyads-nedges);
	for(unsigned int i=0; i< nrebal; i++){
	  ...
	  do{
	    ...
	    for(unsigned int i=0; i<MHp->ntoggles; i++){
	      if(Mtail[i]==Mtail[MHp->ntoggles] && Mhead[i]==Mhead[MHp->ntoggles]){
		repetition = TRUE;
		break;
	      }
	    }
