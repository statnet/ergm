...
ergm_mplecov <- function(pl,nw, fd, m,  theta.mple, invHess,  control,
  ...
  for(i in 1:length(sim.mple)){
    ...
    pl_sim <- ergm.pl(nw=sim.mple[[i]], fd=fd, m=m,
                  theta.offset=init, control=control,verbose=verbose)
  ...
  if(control$MPLE.covariance.method   =="bootstrap"){
    ...
    for(i in 1:length(sim.mple)){
      ...
      glm.sim <- glm(pl_sim$zy ~ .-1 + offset(pl_sim$foffset), 
                    data=data.frame(pl_sim$xmat),weights=pl_sim$wend, family="binomial")
