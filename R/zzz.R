...
.RegisterProposals <- function(){
  ...
  ergm_proposal_table("c", "Bernoulli", "|.dyads|bd&edges",  0, "random", "ConstantEdges")
...
.RegisterKeywords <- function() {
  ...
  ergm_keyword(name="self-loop", short="loop", description="specific to networks with self-loops", popular=FALSE, package="ergm")
}
