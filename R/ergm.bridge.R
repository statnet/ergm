...
ergm.bridge.preproc<-function(object, basis){
  ...
  list(nw=nw, form=form, model=ergm.getmodel(form, nw, drop=FALSE))
}
...
ergm.bridge.llr<-function(object, from, to, nsteps=20, sample.size=10000, burnin=10000, basis=NULL, verbose=FALSE, llronly=FALSE, ...){
  ...
  for(i in seq_len(nsteps)){
    ...
    nw.state<-simulate(form, theta0=theta, nsim=1, basis=basis, statsonly=FALSE, verbose=max(verbose-1,0), burnin=if(i==1) burnin else ceiling(burnin/sqrt(nsteps)), interval=1, ...)
    ergm.update.formula(form,nw.state~.)
    stats[i,]<-apply(simulate(form, theta0=theta, basis=basis, statsonly=TRUE, verbose=max(verbose-1,0), burnin=0, nsim=ceiling(sample.size/nsteps), ...),2,mean)-obs
  }
...
ergm.bridge.dindstart.llk<-function(object, theta, nsteps=20, dind=NULL, theta.dind=NULL,  basis=NULL, llkonly=TRUE, ...){
  ...
  br<-ergm.bridge.llr(form.aug, from=from, to=to, basis=basis, nsteps=nsteps, ...)
