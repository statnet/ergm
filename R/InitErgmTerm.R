...
#' @param levels2 similar to the other levels arguments above and applies to all networks. Optionally allows a factor or character matrix to be specified to group certain levels. Ties specified by the same character or level will be grouped together and summarised by the same statistic. If an empty string is specified, the tie will be ungrouped. Only the upper triangle needs to be specified for undirected networks. `levels2=matrix(c('A', '', NA, 'A'), 2, 2, byrow=TRUE)` on an undirected matrix will group homophilous ties while leaving ties between 1 and 2 ungrouped.
#'
#' @template ergmTerm-general
#'
#' @template ergmTerm-base-dep
