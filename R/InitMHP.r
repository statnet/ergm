...
InitMHP.randomtoggle <- function(arguments, nw, model) {
  MHproposal <- list(name = "randomtoggle", args=NULL)
  if(is.bipartite(nw)){
...
InitMHP.TNT <- function(arguments, nw, model) {
  MHproposal <- list(name = "TNT", args=NULL)
  if(is.bipartite(nw)){
...
InitMHP.ConstantEdges <- function(arguments, nw, model) {
  MHproposal <- list(name = "ConstantEdges", args=NULL)
  if(is.bipartite(nw)){
...
InitMHP.HammingConstantEdges <- function(arguments, nw, model) {
  MHproposal <- list(name = "HammingConstantEdges")
  if(is.bipartite(nw)){
...
InitMHP.formation <- function(arguments, nw, model) {
  MHproposal <- list(name = "formation")
  if(is.bipartite(nw)){
...
InitMHP.formationTNT <- function(arguments, nw, model) {
  MHproposal <- list(name = "FormationTNT")
  if(is.bipartite(nw)){
