stergm.EGMME.OS <- function(theta.form0, theta.diss0, nw, model.form, model.diss, model.mon,
  ...
  eval.optpars <- function(test.G,window,update.jitter){
    ...
    control$GainM[!offsets,] <- robust.inverse(G) %*% suppressWarnings(chol(w,pivot=TRUE)) 
    wscale <-  min(control$SA.gain.max.dist.boost, sqrt(mahalanobis(colMeans(h[,-(1:p),drop=FALSE]),0,cov=robust.inverse(cov(h[,-(1:p),drop=FALSE])),inverted=TRUE)))
    #wscale <- sqrt(mean(diag(control$GainM[!offsets,,drop=FALSE]%*%v%*%t(control$GainM[!offsets,,drop=FALSE])))) / sqrt(mahalanobis(colMeans(oh.last[,-(1:p),drop=FALSE]),0,cov=w,inverted=TRUE))
