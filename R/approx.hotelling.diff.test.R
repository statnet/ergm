...
.ergm.esteq <- function(theta, model, statsmatrix){
  ...
  if(is.mcmc(statsmatrix)){
    ...
    varnames(esteq) <- NVL(names(theta), coef.names.mode(model, FALSE))[!etamap$offsettheta]
  }else  colnames(esteq) <- NVL(names(theta), coef.names.mode(model, FALSE))[!etamap$offsettheta]
