...
#
    if(MCMCparams$parallel==0){
      z <- .C("MCMCDyn_wrapper",
              ...
              # Space for output.
              s.form = as.double(cbind(summary(model.form$formula),matrix(0,nrow=length(model.form$coef.names),ncol=MCMCparams$nsteps))),
              s.diss = as.double(cbind(summary(model.diss$formula),matrix(0,nrow=length(model.diss$coef.names),ncol=MCMCparams$nsteps))),
