...
This document is an automatically generated cross reference for the ergm model terms from the [stanet project](http://statnet.org).  The source for this data and additional descriptions are in the `?ergm.terms` help file or the [ergm manual](https://cran.r-project.org/web/packages/ergm/ergm.pdf). 
...
ergm:::.termMatrix(terms,categories=c('binary', 'valued', 'directed', 'undirected', 'bipartite', 'dyad-independent','operator'),only.include='frequently-used')
```

For convenience, this table lists operator terms: terms that wrap or modify other terms.

```{r,results='asis',echo=FALSE}
ergm:::.termMatrix(terms,categories=c('binary', 'valued', 'directed', 'undirected', 'bipartite', 'dyad-independent','operator'),only.include='operator')
```
