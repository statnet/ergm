...
- `ptr`: raw pointer to the underlying `Network` (or `WtNetwork`) C struct.
...
- `ptr`: raw pointer to the underlying `ModelTerm` (or `WtModelTerm`) C struct.
...
- `ptr`: raw pointer to the underlying `MHProposal` (or `WtMHProposal`) C struct.
- Direct members:
