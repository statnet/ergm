...
@
\begin{center}
\includegraphics[clip=true, trim=0px 0px 0px 0px, width=6in, height=4in]{fig1-1}

\includegraphics[clip=true, trim=0px 200px 0px 0px, width=6in, height=3in]{fig1-2}
\end{center}

Success! Of course, in real life one might have a lot more trial and
error.

Recent changes in the ergm estimation algorithm mean that the MCMC diagnostic plots can no longer be used to ensure that the mean statistics from the model match the observed network statistics. For that functionality, please use the GOF command: \code{gof(fit, GOF=~model).} The plots can still be used to assess MCMC mixing and convergence.
