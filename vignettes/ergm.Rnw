...
We begin with the simplest possible model, the Bernoulli or Erdös-Rényi
...
\[
-1.673\times\mbox{change in the number of ties}+0.139\times\mbox{change in number of triangles}
\]

\begin{itemize}
\item if the tie will not add any triangles to the network, its log-odds
is: $-1.673$.
\item if it will add one triangle to the network, its log-odds is: $-1.673+0.139=-1.534$
\item if it will add two triangles to the network, its log-odds is: $-1.673+0.139\times2=-1.395$
\item the corresponding probabilities are 0.158, 0.177, and 0.199.
\end{itemize}
...
The two estimates for the degree2 coefficient differ considerably.
...
fit <- ergm(magnolia~edges+gwesp(0.25,fixed=T)+nodematch('Grade')+nodematch('Race')+
  nodematch('Sex'),
 control = control.ergm(seed=1,MCMC.samplesize=50000,MCMC.interval=1000),
 verbose=T)
@

<<fig1>>=
png( 'fig1.png', width=900, height=900, units='px')
mcmc.diagnostics(fit)
invisible(dev.off())
@
\includegraphics[clip=true, trim=0px 200px 0px 0px, width=6in]{fig1}
...
of network structure on HIV transmission, see \href{http://statnet.org/movies}{statnet.org/movies}.
...
The statnet suite of packages currently contains many additional features
not covered in this tutorial:
\begin{itemize}
\item tools for fitting dynamic network models (\verb@stergm@, in the
ergm base package)
\item tools for fitting relational event models (\verb@relevent@ package)
\item curved exponential family estimation and simulation (\verb@ergm@
base package)
\item latent space and latent cluster analysis (\verb@latentnet@ package)
\item network permutation models (\verb@netperm@ package)
\item MLE estimation for degree distributions (negative binomial, Poisson,
scale-free, etc.) (\verb@degreenet@ package)
\item analysis of bipartite networks (\verb@network@ package)
\item simulation of bipartite networks with given degree distributions (\verb@networksis@
package)
\item hierarchical ERGMs (\verb@hergm@ package)
\end{itemize}
Any of these not in the ergm base package are in stand-alone packages
that can be downloaded either from CRAN, or from the \verb@statnet@
website. For more detailed information, please visit the \verb@statnet@
webpage (\href{http://statnet.org}{http://statnet.org}).


\subsection{Additional functionality in development:}
\begin{itemize}
\item ERGMs for valued ties \textendash{} expected later 2012
\item Temporal ERGMs (TERGMs) for longitudinal network panel data, and other
temporal extensions \textendash{} expected later 2012
\item Temporally extended (vertex and edge) attributes for TERGMS \textendash{}
expected later 2012
\item Network movie maker: ndTV \textendash{} functionality previewed in
STERGM workshop this year, CRAN release expected later 2012
\end{itemize}

\subsection{Statnet Commons: The development group}
...
Pavel N. Krivitsky <\href{mailto:krivitsky@stat.psu.edu}{krivitsky@stat.psu.edu}>
...
\bibitem[Krivitsky, P.N.(2009)]{thesis}
Krivitsky, P.N.(2009). 
\newblock PhD thesis.
\newblock \emph{University of Washington, Seattle, WA}

\bibitem[Krivitsky, P. N., M. S. Handcock and M. Morris (2011).]{pavel}
...
\newblock \emph{Statistical Methodology, forthcoming}
