...
-->
<!-- to render out this file for preview when not using RStudio:  rmarkdown::render('vignettes/Terms-API.Rmd') -->
...
* Each term has some subset of five functions on the C side: initializers (`i_`), updaters (`u_`), change stats (`c_`), difference stats (`d_`), finalizers (`f_`), and "eXtended" functions (`x_`).
...
1. `ergm_model()` is called.
    ...
    1. `update.ErgmTerm()` when it finds that a term has requested auxiliaries, attaches an integer vector `model$aux.slots` with an element for each auxiliary term requested in the requesting term's input vector.
    1. It calls `ergm.auxstorage()` with the complete model.
        ...
        1. It insets the initialized unique auxiliary terms in `model$terms` and the index (in the unique list) of the auxiliary requested by each statistic in the `model$aux.slots` of the requesting statistic.
        1. For auxiliary itself, the first element of `model$aux.slots` is set to the position of the auxiliary itself.
1. The `ergm_model` is passed to the C code.
...
1. `ModelInitialize()` is called:
    ...
    1. It assigns `model$terms[[i]]aux.slotes` to `mtp->aux_slots`.
    ...
    1. It calls `InitStats()` is called, calling the initializer (`i_` function) of each term or, if not found, an updater (`u_` function) with invalid input (i.e., toggle $(0,0)$) is called in case the term developer prefers a one-function implementation.
